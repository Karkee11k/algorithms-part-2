...
public class SeamCarver {
    ...
    private Picture picture;     // picture to seam-carve
    private double[][] energy;   // energy cache
    ...
    public int[] findVerticalSeam() {
        ...
        TopologicalGrid topological = new TopologicalGrid(width, height);
        ...
        for (Integer[] indices : topological.vertical()) {
            ...
            if (x < width - 1)   relaxV(disTo, edgeTo, x, y, x + 1, y + 1);
            if (x > 0)           relaxV(disTo, edgeTo, x, y, x - 1, y + 1);
            relaxV(disTo, edgeTo, x, y, x, y + 1);
        }
    ...
    public int[] findHorizontalSeam() {
        ...
        for (Integer[] indices : topological.horizontal()) {
            ...
            if (y < height - 1) relaxH(disTo, edgeTo, x, y, x + 1, y + 1);
            if (y > 0)          relaxH(disTo, edgeTo, x, y, x + 1, y - 1);
            relaxH(disTo, edgeTo, x, y, x + 1, y);
        }
    ...
    public void removeVerticalSeam(int[] seam) {
        ...
        picture = pic;
        this.energy = new double[width() - 1][height()];
    }
    ...
    // relax the edge for vertical DAG
    private void relaxV(double[][] disTo, int[][] edgeTo, int x, int y, int x1, int y1) {
        double e = energy(x1, y1);
        if (disTo[x][y] + e < disTo[x1][y1]) {
            disTo[x1][y1] = disTo[x][y] + e;
            edgeTo[x1][y1] = x - x1;
        }
    }

    // relax the edge for horizontal DAG
    private void relaxH(double[][] disTo, int[][] edgeTo, int x, int y, int x1, int y1) {
        double e = energy(x1, y1);
        if (disTo[x][y] + e < disTo[x1][y1]) {
            disTo[x1][y1] = disTo[x][y] + e;
            edgeTo[x1][y1] = y - y1;
    ...
    public static void main(String[] args) {
        ...
        StdOut.print(seamCarver.width() + "x" + seamCarver.height());
